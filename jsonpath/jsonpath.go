...
// Execute bounds data into template and writes the result.
func (j *JSONPath) ExecuteJson(wr io.Writer, data interface{}) error {
	fullResults, err := j.FindResults(data)
	if err != nil {
		return err
	}
	for ix := range fullResults {
		if err := j.PrintResultsJson(wr, fullResults[ix], len(fullResults), ix); err != nil {
			return err
		}
	}
	return nil
...
// PrintResults writes the results into writer
func (j *JSONPath) PrintResultsJson(wr io.Writer, results []reflect.Value, length int, ix int) error {
	...
	for i, r := range results {
		...
		text, err := j.evalToJson(r)
		if err != nil {
			return err
		}

		if string(text) == "[" && ix > 0 { //from range and not last entry
			textFull = append(textFull, ',')
		}
		textFull = append(textFull[:], text[:]...)
		if i != len(results)-1 {
			textFull = append(textFull, ',')
		}
		if _, err = wr.Write(textFull); err != nil {
			return err
		}
...
// evalToText translates reflect value to corresponding text
func (j *JSONPath) evalToJson(v reflect.Value) ([]byte, error) {
	iface, ok := PrintableValue(v)
	if !ok {
		return nil, fmt.Errorf("can't print type %s", v.Type())
	}
	...
	default:
		...
		if err != nil {
			return nil, fmt.Errorf("can't marshal type %s", v.Type())
		}
	}
